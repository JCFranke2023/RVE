...
csv_folder = r"C:\\Jonas Diffusions Simulationen\\Poren Listen"
...
results_folder = os.path.join(r"C:\Jonas Diffusions Simulationen\Ergebnisse", f"{current_datetime}")
os.makedirs(results_folder, exist_ok=True)

for csv_file in csv_files:
    filename = os.path.join(csv_folder, csv_file)
    try:
        number_part = csv_file.split('list_')[1].split('.')[0]  # Nimmt den Teil nach 'list_' bis zum naechsten '_'
    except IndexError:
    ...
    job_name = f"MassDiffusionJob_{number_part}"
    odb_file = f"{job_name}.odb"

    print(f"Start Simulation {x}. with File {csv_file} and naming it {odb_file}")
    ...
    ModelName = f"RVE_Foam_{number_part}"
    ...
    cube_width = max_corner + 2 * max_radius
    cube_height = 5
    cube_length = max_corner +  2 * max_radius

    print(f"Cube with the Dimensions: {cube_width}, {cube_height}, {cube_length} has been created.")
    ...
    ########## Im Folgenden Abschnitt werden die Poren erstellt und direkt partitioniert, um eine gute Meshbarkeit zu gewaehrleisten ###########
    ...
    for Inclusion in Inclusions:
        ...
        solubility_coefficient = 1 * 1e-18  # Loeslichkeit der Poren
        m.materials[material_name].Solubility(
...
##### In diesem Abschnitt wird der Wuerfel mit den Poren vereint und die Materialdaten usw. neu zugeordnet ######
    ...
    #Neuzuweisung der Sections und der Materialdaten fuer den Wuerfel
    cube_material_name = 'CubeMaterial'
    cube_section_name = 'CubeSection'
    define_cube_material(m, cube_material_name)
    create_section(m, cube_section_name, cube_material_name)
    cube_region = combined_part.Set(
        cells=combined_part.cells.findAt(((cube_width / 200, cube_height / 200, cube_length / 200),)), name='CubeRegion') #Hier wird etilt, damit keine Pore mit den Materialdaten erwischt wird
    ...
    mdb.models[ModelName].MassDiffusionStep(
    ...
    ####### In diesem Abschnitt werden die Randbedingungen fuer die INLET und OUTLET FACE erstellt #######
    ...
    inlet_face = assembly.instances['CombinedGeometry-1'].faces.findAt(
        ((cube_width / 2,cube_height, cube_length / 2),))
    ...
    mdb.models[ModelName].ConcentrationBC(
    ...
    #Bauteil refferenzieren
    combined_part = mdb.models[ModelName].parts['CombinedGeometry']
    ...
    def PeriodicBound3D(mdb, NameModel, NameSet, LatticeVec, scale):
        import time
        start1 = time.time()
    ...
    mat_part = mdb.models[ModelName].parts['CombinedGeometry']
    n = mat_part.nodes
    nodes = n.getByBoundingBox(zMin=-max_radius-0.001, yMin=-max_radius-0.001, xMin=-max_radius-0.001,
    zMax=-max_radius+0.001, yMax=cube_height, xMax=cube_width) + \
    n.getByBoundingBox(zMin=-max_radius-0.001+cube_length, yMin=-max_radius-0.001, xMin=-max_radius-0.001,
    zMax=-max_radius+0.001+cube_length, yMax=cube_height, xMax=cube_width) + \
    n.getByBoundingBox(zMin=-max_radius-0.001, yMin=-max_radius-0.001, xMin=-max_radius-0.001,
    zMax=cube_length, yMax=cube_width, xMax=-max_radius+0.001) + \
    n.getByBoundingBox(zMin=-max_radius-0.001, yMin=-max_radius-0.001, xMin=-max_radius-0.001+cube_width,
    zMax=cube_length, yMax=cube_width, xMax=-max_radius+0.001+cube_width)

    mat_part.Set(nodes=nodes, name='NSET_Mat_Surface')

    #Definieren der vorgegebenen Abstaende von Knoten und Partnerknoten
    LatticeVec = [[cube_width, 0, 0], [0, 0, cube_length]]
    ...
    scale_factor = 2
    #Funktionsaufruf
    PeriodicBound3D(mdb, ModelName, "NSET_Mat_Surface", LatticeVec, scale=scale_factor)
    ...
    mdb.Job(name=job_name, model=ModelName, description=f"Simulation {number_part}")
    ...
    print(f"Simulation {x}. was running {laufzeit:.2f} seconds.")

    x = x + 1

print(f"All {x-1} Simulations finished successfully")
